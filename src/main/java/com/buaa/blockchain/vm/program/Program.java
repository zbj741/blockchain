...
import com.buaa.blockchain.vm.*;
import com.buaa.blockchain.vm.client.Repository;
import com.buaa.blockchain.vm.program.exception.*;
...
import com.buaa.blockchain.vm.spec.PrecompiledContract;
import com.buaa.blockchain.vm.spec.PrecompiledContractContext;
...
import static com.buaa.blockchain.vm.utils.BigIntegerUtil.isNotCovers;
import static com.buaa.blockchain.vm.utils.BigIntegerUtil.transfer;
import static com.buaa.blockchain.vm.utils.ByteArrayUtil.*;
...
public class Program {
    ...
    public Program(byte[] ops, ProgramInvoke programInvoke) {
        this(ops, programInvoke, new DefaultSpec());
    }
    ...
    private Transaction addInternalTx(OpCode type, byte[] from, byte[] to, long nonce, BigInteger value,
                                      byte[] data, long gas) {

        int depth = getCallDepth();
        int index = getResult().getTransactions().size();

        Transaction tx = new Transaction(depth, index, type.name(), from, to, nonce, value, data, gas, getGasPrice().value());
        getResult().addTransaction(tx);
    ...
    private ProgramResult createContractImpl(DataWord value, byte[] programCode, byte[] newAddress, long gas) {
        ...
        Transaction internalTx = addInternalTx(OpCode.CREATE, senderAddress, EMPTY_BYTE_ARRAY,
        ...
        if (result.getException() == null && !result.isRevert()) {
            ...
            long storageCost = getLength(code) * spec.getGasTable().getCREATE_DATA();
    ...
    private ProgramResult callContractImpl(MessageCall msg, PrecompiledContract contract) {
        ...
        Transaction internalTx = addInternalTx(msg.getType(), senderAddress, contextAddress,
    ...
    public DataWord getBlockHash(int index) {
        return index < this.getBlockNumber().longValue()
                ...
                ? DataWord.of(this.invoke.getRepository().getBlockHashByNumber(index))
                : DataWord.ZERO;
    }
